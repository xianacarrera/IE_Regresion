...
```{r icbeta0mano}
beta0.cuantil <- qt(1-alfa/2, df=n-2); beta0.cuantil       # Cuantil 1-alfa/2 de T de Student con n-2 grados de libertad
...
### $\sigma^2$
Por último, para el intervalo de confianza de la varianza del error, usaremos 
...
Si no consiguiéramos demostrar la hipótesis alternativa para el primer contraste, tendríamos que considerar un modelo más sencillo, que no solo facilitaría el análisis estadístico, sino que es más correcto y realista de cara a las interpretaciones que podamos hacer. En el caso del segundo contraste, no demostrar la hipótesis alternativa equivaldría a invalidar nuestro modelo, pues significaría que no hay regresión.

En primer lugar, calculamos los contrastes de forma manual. Esto se hará a partir de los estadísticos de contraste basados en los pivotes empleados para obtener los intervalos de confianza del ejercicio 3, teniendo en cuenta que, bajo las respectivas hipótesis nulas, $\beta_0 = 0$ y $\beta_1 = 0$:
```{r signifmano}
...
summary(modelo)
```
En concreto, los valores relevantes son el \texttt{t-value} y el \texttt{Pr(>|t|)} de las filas \texttt{(Intercept)} y \texttt{X} que se corresponden al valor observado del estadístico de contraste y su p-valor, para el intercepto $\beta_0$ y la pendiente $\beta_1$ respectivamente.
