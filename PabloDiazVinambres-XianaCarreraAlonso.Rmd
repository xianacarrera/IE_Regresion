...
$$
\overline{x} = \frac{1}{n} \sum_{i=1}^n x_i = 4.268 \;\;\text{unidades de X}, \quad
\overline{Y} = \frac{1}{n} \sum_{i=1}^n Y_i = -0.1815 \;\;\text{unidades de Y}

LÍNEA 2fdsfdskjllllllllllllllllllllllllllllllllllllllllllllllllllllll
S_x^2 = \frac{1}{n}\sum_{i=1}^n (x_i - \overline{x}) = 2.844383  \;\;\text{(unidades de X)}^2 
S_Y^2 = \frac{1}{n}\sum_{i=1}^n (Y_i - \overline{Y}) = 9.365897  \;\;\text{(unidades de Y)}^2 

LÍNEA 3fdsfdskjllllllllllllllllllllllllllllllllllllllllllllllllllllll
S_x = \sqrt{S_x^2} = 2.844383
S_Y = \sqrt{S_Y^2} = 3.060375
$$
...
El modelo obtenido almacena otros datos de interés, tales como los residuos (\texttt{modelo\$residuals}), los valores ajustados de la Y (\texttt{modelo\$fitted.values}), etc.

En los siguientes ejercicios, analizaremos en profundidad diferentes características de este modelo y obtendremos inferencia a partir del mismo. Nótese que para que las conclusiones extraídas en estos ejercicios tengan validez, deberemos suponer que se cumplen las hipótesis de linealidad, homocedasticidad y normalidad e independencia de los errores. Las comprobaremos de forma precisa en el ejercicio 7, pero de no ser válida alguna de ellas, tendríamos que revisar y descartar multitud de resultados.
...
$$
generando así el intervalo:
$$
(\tilde{Y}_0 - t_{n-2, \frac{\alpha}{2}} \:\frac{\hat{\sigma}}{\sqrt{n_0}}, \tilde{Y}_0 + t_{n-2, \frac{\alpha}{2}} \:\frac{\hat{\sigma}}{\sqrt{n_0}})
$$
de esta forma, podemos calcular como sigue:
```{r mediacondmano}
...
nuevosValores.intervalosMediaCond = c(nuevosValores.extrInfMediaCond, nuevosValores.extrSupMediaCond); nuevosValores.intervalosMediaCond
```
NO FUNCIONA (EXCEPTO LOS Y0TILDE) NO SE QUE LE PASA
...
nuevosValores.intervalosPred = c(nuevosValores.extrSupPred, nuevosValores.extrSupPred); nuevosValores.intervalosPred
