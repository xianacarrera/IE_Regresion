...
\begin{enumerate}
  ...
  \item \texttt{std. error}: el error típico estimado para el intercepto y para la pendiente, respectivamente.
  \item \texttt{t value}: el cociente entre la columna \texttt{estimate} y la columna \texttt{std. error}.
  \item \texttt{p value}: el p-valor de los contrastes de signifcación asociados.
\end{enumerate}
...
```{r dispersionReg, echo=FALSE, fig.align='center'}
...
```{r graftstudent, echo = FALSE, fig.align='center'}
...
```{r grafchi, echo = FALSE, fig.align='center'}
...
segments(x0 = var.error.cuantilinferior, y0 = -0.05,
        ...
        y1 = dchisq(var.error.cuantilinferior, df = n - 2) + 0.05, col = "red")
...
99 contendrían al parámetro asociado. No hay ninguna función estándar para la automatización del intervalo de confianza para la varianza del error.
...
```{r pvalores}
beta0.pvalor <- dt(beta0.t, df = n - 2)
beta0.pvalor
beta1.pvalor <- dt(beta1.t, df = n - 2)
beta1.pvalor
```
...
```{r dispersionresiduos, echo = FALSE, fig.align='center'}
par(mfrow = c(1, 2))
...
```{r representacionnormalidad, echo = FALSE, fig.align='center'}
