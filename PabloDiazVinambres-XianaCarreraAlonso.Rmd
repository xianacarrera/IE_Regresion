...
\begin{gather*}
\text{Rechazar } H_0: \beta_0 = 0 \text{ si } |T_0| = \frac{|\hat\beta_0|}{\hat\sigma\sqrt{\frac{1}{n}+\frac{\overline{x}^2}{nS_x^2}}} \in T_{n-2} > t_{n-2, \alpha/2} \\
...
Podemos también ver los respectivos p-valores (el menor nivel de significación para el cual podemos aceptar la hipótesis nula), como $\mathbb{P}(|T| > T_0)$ y $P$ y del área que queda a la derecha de los estadísticos. Cogemos el área derecha al ser ambos estadísticos positivos. Además, tendremos que multiplicar ambas probabilidades por 2, al estar considerando un contraste bilateral.
```{r pvalores}
beta0.pvalor <- 2*(1-pt(beta0.t, df = n - 2))
beta0.pvalor
beta1.pvalor <- 2*(1-pt(beta1.t, df = n - 2))
beta1.pvalor
```
...
qqPlot(residuos.estan, main = "Diagrama cuantil-cuantil", ylab = "Residuos estandarizados", xlab = "Residuos teóricos")
```
