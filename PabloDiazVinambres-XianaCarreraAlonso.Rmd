...
```{r lecturadatos}
#setwd(dirname(rstudioapi::getActiveDocumentContext()$path)) # Configurar wd a la carpeta actual (solo funciona en RStudio) 

# Leemos las columnas X e Y47 del archivo de datos y los ponemos en sendas variables X e Y
datos <- read.table("datos_trabajo_temas6y7.txt", header=T, sep=" ", dec=".")
datos = datos[,c("X", "Y47")] # Filtramos el archivo, solo nos interesan las columnas X e Y47
str(datos)
summary(datos)
head(datos)
str(datos)
...
En el siguiente apartado, En primer lugar, establecemos el nivel de significación $\alpha$.
```{r alfa}
alfa <- 1 - 0.99
```
